...
def _resolve_analyze_inputs(cfg, ctx=None) -> tuple[Path, Path, Path, Path]:
    if ctx is not None:
        ...
        if analyze_artifact:
            ...
            temp_band_summary = analyze_artifact.get("temp_band_summary_csv")
            summary_json = analyze_artifact.get("summary_json")
            if (
                hourly_profile
                and high_demand_share
                and temp_band_summary
                and summary_json
            ):
                return (
                    Path(hourly_profile),
                    Path(high_demand_share),
                    Path(temp_band_summary),
                    Path(summary_json),
                )
...
def run_report(cfg, ctx=None) -> dict:
    ...
    temp_band_summary = pd.read_csv(temp_band_summary_path)
    ...
    if write_summary_markdown:
        ...
        lines.extend(
            [
                ...
                "",
                "| Temperature Band | Mean Rentals |",
                "| --- | --- |",
            ]
        )
        for _, row in temp_band_summary.iterrows():
            lines.append(f"| {row['temp_band']} | {row['mean_rentals']:.2f} |")
        lines.extend(["", "Generated by `run_report` from analyze-stage artifacts."])
