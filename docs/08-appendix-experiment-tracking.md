...
- params: `analysis.threshold`, `prepare.keep_outliers`
- metrics: `prepare.rows_out`, `analyze.calculated_metric`
...
def _log_selected_params(cfg) -> None:
    mlflow.log_param("analysis.threshold", float(cfg.analysis.threshold))
    mlflow.log_param("prepare.keep_outliers", bool(cfg.prepare.keep_outliers))
...
def _log_stage(stage_name: str, artifact: dict) -> None:
    ...
    if stage_name == "analyze" and artifact.get("calculated_metric") is not None:
        mlflow.log_metric("analyze.calculated_metric", float(artifact["calculated_metric"]))
